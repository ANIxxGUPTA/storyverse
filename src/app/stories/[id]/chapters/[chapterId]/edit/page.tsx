...
export default function EditChapterPage({
  ...
  const { value: title, setValue: setTitle, clearAutosave: clearTitleAutosave } = useAutosave("", { key: `edit-title-${chapterId}` });
  const { value: content, setValue: setContent, isSaved, lastSaved, clearAutosave: clearContentAutosave } = useAutosave("", { key: `edit-content-${chapterId}` });
  ...
  useEffect(() => {
    ...
    async function fetchChapterDetails() {
      try {
        ...
        // Only set from DB if we don't have an autosave
        if (!localStorage.getItem(`edit-title-${chapterId}`)) {
          setTitle(data.chapter.title);
        }
        if (!localStorage.getItem(`edit-content-${chapterId}`)) {
          setContent(data.chapter.content);
        }
        setChapterStatus(data.chapter.status || "draft");
      } catch (err) {
        console.error(err);
      }
    }
  }, [id, chapterId, session, authStatus, router, setTitle, setContent]);
          ...
          <div className={`rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-8 shadow-sm ${isAiOpen ? 'lg:col-span-2' : ''}`}>
            <div className="flex flex-col gap-2 pb-6 border-b border-zinc-200 dark:border-zinc-800">
              ...
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold font-serif text-zinc-900 dark:text-white">Edit Chapter</h1>
              </div>
            </div>
