...
export default function Search() {
  ...
  // If semantic search is toggled ON and we have a query, use the semantic search API.
  // Otherwise, use the standard /api/stories list with genre filters
  let url = '/api/stories?';
  if (isSemantic && debouncedQuery) {
    ...
    if (debouncedQuery) params.append("search", debouncedQuery); // If the standard API supports search
    url += params.toString();
  }
  ...
  if (selectedTag) {
    stories = stories.filter((s: Story) => s.tags?.includes(selectedTag));
  }
  
  if (selectedSort === "recent") {
    stories.sort((a: Story, b: Story) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  } else if (selectedSort === "views") {
    stories.sort((a: Story, b: Story) => (b.views || 0) - (a.views || 0));
  } else if (selectedSort === "likes") {
    stories.sort((a: Story, b: Story) => (b.likes?.length || 0) - (a.likes?.length || 0));
  }
          ...
          <form onSubmit={handleSearchSubmit} className="flex gap-2">
            <div className="relative flex-grow">
              ...
              {isSemantic ? (
                ...
                <SearchIcon className="absolute left-3.5 top-3.5 h-4 w-4 text-zinc-500" />
              )}
            </div>
            <button
              type="button"
